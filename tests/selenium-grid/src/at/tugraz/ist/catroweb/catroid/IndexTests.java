...
public class IndexTests extends BaseTest {
	...
	public void index() throws Throwable {
		try {
			...
			assertTrue(session().isElementPresent(
					"xpath=//img[@class='catroidLettering']"));
			// test logo link
			assertTrue(session()
					.isElementPresent("xpath=//div[@class='webHeadLogo']"));
			session().click("xpath=//div[@id='aIndexWebLogoLeft']");
			ajaxWait();
			// test catroid download link
			assertTrue(session().isElementPresent(
					"xpath=//a[@id='aIndexWebLogoMiddle']"));
	...
	public void pageNavigation() throws Throwable {
		try {
			...
			for (int i = 0; i < Config.PROJECT_PAGE_LOAD_MAX_PROJECTS
					* (Config.PROJECT_PAGE_SHOW_MAX_PAGES + 1); i++) {
				System.out.print(".");
			...
			assertTrue(session().isTextPresent(
					CommonStrings.NEWEST_PROJECTS_PAGE_NEXT_BUTTON));
			int i = 0;
			for (i = 0; i < Config.PROJECT_PAGE_SHOW_MAX_PAGES; i++) {
				session().click("moreProjects");
				ajaxWait();
				assertRegExp("^" + CommonStrings.WEBSITE_TITLE + " - "
						+ CommonStrings.NEWEST_PROJECTS_PAGE_TITLE + " - " + (i + 2) + "$",
						session().getTitle());
			}

			assertTrue(session().isVisible("fewerProjects"));
			assertTrue(session().isTextPresent(
					CommonStrings.NEWEST_PROJECTS_PAGE_PREV_BUTTON));
			session().click("fewerProjects");
			ajaxWait();
			assertRegExp("^" + CommonStrings.WEBSITE_TITLE + " - "
					+ CommonStrings.NEWEST_PROJECTS_PAGE_TITLE + " - " + (i) + "$",
					session().getTitle());
			...
			ajaxWait();
			assertRegExp("^" + CommonStrings.WEBSITE_TITLE + " - "
					+ CommonStrings.NEWEST_PROJECTS_PAGE_TITLE + " - " + (1) + "$",
					session().getTitle());
		} catch (AssertionError e) {
